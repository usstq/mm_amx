    ...
    */
    ...
    //for (int k = 0; k < Ktiles; k++) {
      ...
      for(int c = 0; c < 6; c++) {
        ...
        for(int i = 0; i < 1024; i+=64) prefetcht0(ptr[reg_B_addr + 4096 + i + c*1024]);
...
int amx_jit16x96(int K, int times = -1000) {
  const int M = 16;
  const int N = 96;
  tensor2D<ov::bfloat16> A(M, K,
                           true);  // ensure stride of A matrix is multiple of
  ...
  tensor2D<float> C0(M, N, true);  // reference result
  tensor2D<float> C1(M, N, true);  // actual result
  Linear16x96_AMX mm_jit(K);
  TileConfigScope tcfg(mm_jit.tile_config());

  for (int k = 0, i = 0; k < K; k += 32) {
    for(int n = 0; n < 96; n+=16) {
        amx_kernel::functional::transpose_epi32_16x16(&BPacked[i * 16 * 32],
                                                    &Bt(n, k), Bt.stride);
        i++;
    }
...
int amx_jit16x64(int K, int times = -1000) {
  const int M = 16;
  const int N = 64;
  tensor2D<ov::bfloat16> A(M, K,
                           true);  // ensure stride of A matrix is multiple of
...
public:
 ...
 Xbyak::Reg64 reg_addrB = abi_param3;
 Xbyak::Reg64 reg_strideB = abi_param4;
...
void profile_tileload() {
  ...
  tensor2D<ov::bfloat16> B(M, K,true);
    InstProfiler p;
    TileConfigScope tcfg(p.tile_config());

  timer.tag(__func__,A.stride, "(M=", M, ",N=", N, ",K=", K, ")")(100, [&]() {
    p(&A[0], A.stride, &B[0], 64, 1000);
  }
  );
  timer.tag(__func__,64, "(M=", M, ",N=", N, ",K=", K, ")")(100, [&]() {
    p(&A[0], 64, &B[0], 64, 1000);
  }
  );
...
int main(int argc, const char* argv[]) {
  ...
  //profile_tileload(); profile_tileload(); profile_tileload(); return 0;
  std::cout << "===============================BF16========================\n";
    amx_mm(32, 32, 128);
    amx_jit32x32(128);
    amx_mm(32, 32, 128);
